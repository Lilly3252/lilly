import type { event, SlashCommand } from '#type/index.js';
import fs from 'fs';
...
export default class Utils {
	...
	list(a: unknown[], b = 'and') {
		...
		function list() {
			const c = a.length;
			return c === 0 ? '' : c === 1 ? a[0] : `${a.slice(0, -1).join(', ')}${c > 1 ? `${c > 2 ? ',' : ''} ${b} ` : ''}${a.slice(-1)}`;
		}
	}
	...
	isClass(a: { prototype: unknown; toString: () => string }) {
		return isClass();
		function isClass() {
			return typeof a == 'function' && typeof a.prototype == 'object' && a.toString().substring(0, 5) === 'class';
		}
	}
	...
	emojify(mode: boolean) {
		return Boolean(mode) ? '✅' : '❌';
	}
	isEnabled(name: boolean) {
		return Boolean(name) ? 'Enabled' : 'Disabled';
	}
	...
	async loadCommands() {
		...
		for (const folder of modules) {
			const slashyFiles = fs.readdirSync(`./dist/src/commands/${folder}/`).filter((file: string) => file.endsWith('.js'));
	...
	async loadEvents() {
		const eventFiles = fs.readdirSync(`./dist/src/events`).filter((file) => file.toString().endsWith('.js'));
