  ...
  const UserInfo = new MessageEmbed()
    ...
    .addField(
      ...
      [
        ...
        `**❯ ID:** ${member.id}`,
        `**❯ Flags:** ${
          flag.length ? flag.map((a) => flags[a]).join(", ") : "None"
        }`,
  ...
  const ServerInfo = new MessageEmbed()
    ...
    .addField(
      ...
      [
        ...
        `**❯ Text Channels:** ${
          d.filter((channel) => "GUILD_TEXT" === channel.type).size
        }`,
        `**❯ Voice Channels:** ${
          d.filter((channel) => "GUILD_VOICE" === channel.type).size
        }`,
        `**❯ Stage Channels:** ${
          d.filter((channel) => "GUILD_STAGE_VOICE" === channel.type).size
        }`,
        `**❯ Boost Count:** ${
    ...
    .addField(
      ...
      [
        ...
        `**❯ Do Not Disturb:** ${
          member.filter((guildmember) => "dnd" === guildmember.presence?.status)
            .size
        }`,
        `**❯ Offline:** ${
          member.filter(
        ...
        `**❯ No presence detected:** ${
          member.filter((guildmember) => null === guildmember.presence).size
        }`,
        "\u200B",
      ].join("\n")
    )
...
module.exports.ChannelEmbed = function (
  ...
  const ChanEmbeds = new MessageEmbed()
    ...
    .addField(
      ...
      ` ${
        channeltypes.length
          ? channeltypes.map((a) => ChannelType[a])
