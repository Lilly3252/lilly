...
module.exports = {
  ...
  async run(interaction) {
    ...
    let role = interaction.guild.roles.cache.find((role) => "Muted" === role.name);
    if (!role)
      try {
    ...
    member.roles.add(role.id).then(() => {
      member.send(`Hello, you have been muted in ${interaction.guild.name} for: ${reason}`)
        .catch((err) => console.log(err)),
        interaction.reply({ content: `${member.user.username} was successfully muted.`, ephemeral: true });
    });


    const i = guild_log.logchannelID;
    i && null !== i &&
      interaction.client.channels.cache.get(i).send({ embeds: [Embed.AdminEmbed(interaction, member, reason)] });
  },
};
