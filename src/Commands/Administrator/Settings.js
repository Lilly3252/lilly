...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) => subcommand.setName("messagedelete").setDescription("set the DeleteMessages event ON or OFF"))
	...
	async run(interaction) {
			...
			const d = new MessageEmbed()
				...
				.setDescription(
					[
						...
						`**❯ WelcomeMessage:**${
							guild_db.PersonalizedWelcomeMessage ? guild_db.PersonalizedWelcomeMessage : "Not defined yet"
						}`,
		...
		if (interaction.options.getSubcommand() === "anti-raid") {
			"on" === b[1] && (await c.updateOne({ antiRaidMode: !0 }), interaction.reply("\u2705 AntiRaid Mode enable.")),
				"off" === b[1] && (await c.updateOne({ antiRaidMode: !1 }), interaction.reply("\u274C AntiRaid Mode disable."));
		}
		// MessageUpdate event
		if (interaction.options.getSubcommand() === "messageupdates") {
			"true" === b[1] && (await c.updateOne({ messageUpdateMode: !0 }), interaction.reply("\u2705 MessageUpdate has been enable.")),
				"false" === b[1] &&
					(await c.updateOne({ messageUpdateMode: !1 }), interaction.reply("\u274C MessageUpdate has been disable."));
		}
		//MessageDelete Event
		if (interaction.options.getSubcommand() === "messagedelete") {
			"true" === b[1] && (await c.updateOne({ messageDeleteMode: !0 }), interaction.reply("\u2705 MessageDelete has been enable.")),
				"false" === b[1] &&
					(await c.updateOne({ messageDeleteMode: !1 }), interaction.reply("\u274C MessageDelete has been disable."));
		}
		//MessageDeleteBulk Event
		if (interaction.options.getSubcommand() === "messagedeletebulk") {
			"true" === b[1] &&
				(await c.updateOne({ messageBulkDeleteMode: !0 }), interaction.reply("\u2705 messageDeleteBulk has been enable.")),
				"false" === b[1] &&
					(await c.updateOne({ messageBulkDeleteMode: !1 }), interaction.reply("\u274C MessageDeleteBulk has been disable."));
		}
		//WelcomeMessage Setup
		if (interaction.options.getSubcommand() === "welcomemessage") {
			const e = b[1];
			isNaN(e) && "false" === b[1] && (await c.updateOne({ welcomechannelID: null })),
				isNaN(e) && "false" !== b[1]
					? a.channel.send("\u274C You need to give me a channelID to set this setting.").then((a) => a.delete({ timeout: 1e4 }))
					: await c.updateOne({ welcomechannelID: e }).then(() => interaction.reply(`✅ Welcome Channel has been set to ${e}`));
		}
		//ModLog channelID setup
		if (interaction.options.getSubcommand() === "modlog") {
			const f = b[1];
			isNaN(f) && "false" === b[1] && (await c.updateOne({ logchannelID: null })),
				isNaN(f) && "false" !== b[1]
					? a.channel.send("\u274C You need to give me a channelID to set this setting.").then((a) => a.delete({ timeout: 1e4 }))
					: await c.updateOne({ logchannelID: f }).then(() => interaction.reply(`✅ ModLog Channel has been set to ${f}`));
		}
		//ModRole ID Setup
		if (interaction.options.getSubcommand() === " modrole") {
			...
			isNaN(h) && "false" === b[1]
				...
				: (await c.updateOne({ moderatorRoleID: h }), interaction.reply(`✅ ModRole has been set to ${h}`));
		}
		// WelcomeMessage Setup
		if (interaction.options.getSubcommand() === "welcomemessage") {
			...
			isNaN(i) && "false" === b[1]
				...
				: (await c.updateOne({ PersonalizedWelcomeMessage: i }), interaction.reply(`✅ Welcome Message has been set to ${i}`));
		}
	}
};
