...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("settings").setDescription("show or add settings")
  ...
  async run(interaction) {
    ...
    if (interaction.options.getSubcommand() === "showsettings") {
      interaction.reply({ embeds: [Embed.SettingEmbed(interaction, guild_db)], ephemeral: true });
    }
    //**SUB COMMANDS */
    if (interaction.options.getSubcommand() === "anti-raid") {
      ...
      true === choices &&
        (await guild_db.updateOne({ antiRaidMode: true }),
          interaction.reply({ content: "\u2705 AntiRaid Mode enable.", ephemeral: true })),
        false === choices &&
        (await guild_db.updateOne({ antiRaidMode: false }),
          interaction.reply({ content: "\u274C AntiRaid Mode disable.", ephemeral: true }));
    }
    // MessageUpdate event
    if (interaction.options.getSubcommand() === "messageupdates") {
      ...
      true === choices &&
        (await guild_db.updateOne({ messageUpdateMode: true }),
          interaction.reply({ content: "\u2705 MessageUpdate has been enable.", ephemeral: true })),
        false === choices &&
        (await guild_db.updateOne({ messageUpdateMode: false }),
          interaction.reply({ content: "\u274C MessageUpdate has been disable.", ephemeral: true }));
    }
    //MessageDelete Event
    if (interaction.options.getSubcommand() === "messagedelete") {
      ...
      true === choices &&
        (await guild_db.updateOne({ messageDeleteMode: true }),
          interaction.reply({ content: "\u2705 MessageDelete has been enable.", ephemeral: true })),
        false === choices &&
        (await guild_db.updateOne({ messageDeleteMode: false }),
          interaction.reply({ content: "\u274C MessageDelete has been disable.", ephemeral: true }));
    }
    ...
    if (interaction.options.getSubcommand() === "messagedeletebulk") {
      ...
      true === choices &&
        (await guild_db.updateOne({ messageBulkDeleteMode: true }),
          interaction.reply({ content: "\u2705 messageDeleteBulk has been enable.", ephemeral: true })),
        false === choices &&
        (await guild_db.updateOne({ messageBulkDeleteMode: false }),
          interaction.reply({ content: "\u274C MessageDeleteBulk has been disable.", ephemeral: true }));
    }
    ...
    if (interaction.options.getSubcommand() === "welcomechannel") {
      ...
      isNaN(e) && "false" === e
        ...
        : (await guild_db.updateOne({ welcomechannelID: e }),
          interaction.reply({ content: `✅ Welcome Channel has been set to ${e}`, ephemeral: true }));
    }
    ...
    if (interaction.options.getSubcommand() === "modlog") {
      ...
      isNaN(f) &&
        ...
        isNaN(f) && "false" !== i
          ? interaction.reply(
            { content: "\u274C You need to give me a channelID to set this setting.", ephemeral: true }
          )
          : await guild_db
            .updateOne({ logchannelID: f })
            .then(() =>
              interaction.reply({ content: `✅ ModLog Channel has been set to ${f}`, ephemeral: true })
            );
    }
    //ModRole ID Setup
    if (interaction.options.getSubcommand() === "modrole") {
      ...
      isNaN(h) && "false" === h
        ...
        : (await guild_db.updateOne({ moderatorRoleID: h }),
          interaction.reply({ content: `✅ ModRole has been set to ${h}`, ephemeral: true }));
    }
    ...
    if (interaction.options.getSubcommand() === "welcomemessage") {
      ...
      isNaN(i) && "false" === i
        ...
        : (await guild_db.updateOne({ PersonalizedWelcomeMessage: i }),
          interaction.reply({ content: `✅ Welcome Message has been set to ${i}`, ephemeral: true }));
    }
  },
};