...
export const slashy: SlashCommand['slashy'] = new SlashCommandBuilder()
	...
	.addNumberOption((option) =>
		option.setName('number').setDescription('number of messages from 1-99').setRequired(true),
	)
...
export const run: SlashCommand['run'] = async (interaction: ChatInputCommandInteraction<'cached'>): Promise<void> => {
	...
	if (count! > 100) return void interaction.reply({ content: SYSTEM.ERROR.ADMIN.VALID_AMOUNT, ephemeral: true });
	try {
