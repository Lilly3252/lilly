...
module.exports = {
    ...
    //**Need to be tested */
		if (interaction.options.getSubcommand() === "anti-raid") {
			...
			true === choices && (await guild_db.updateOne({ antiRaidMode: !0 }), interaction.reply("\u2705 AntiRaid Mode enable.")),
      false === choices && (await guild_db.updateOne({ antiRaidMode: !1 }), interaction.reply("\u274C AntiRaid Mode disable."));
		}
		// MessageUpdate event
    //**Need to be tested */
		if (interaction.options.getSubcommand() === "messageupdates") {
			const choices = interaction.options.getBoolean("choice");
			true === choices && (await guild_db.updateOne({ messageUpdateMode: !0 }), interaction.reply("\u2705 MessageUpdate has been enable.")),
			false === choices && (await guild_db.updateOne({ messageUpdateMode: !1 }), interaction.reply("\u274C MessageUpdate has been disable."));
		}
		//MessageDelete Event
    //**Need to be tested */
		if (interaction.options.getSubcommand() === "messagedelete") {
			const choices = interaction.options.getBoolean("choice");
			true === choices && (await guild_db.updateOne({ messageDeleteMode: !0 }), interaction.reply("\u2705 MessageDelete has been enable.")),
			false === choices && (await guild_db.updateOne({ messageDeleteMode: !1 }), interaction.reply("\u274C MessageDelete has been disable."));
		}
		//MessageDeleteBulk Event
    //**Need to be tested */
		if (interaction.options.getSubcommand() === "messagedeletebulk") {
			const choices = interaction.options.getBoolean("choice");
			true === choices && (await guild_db.updateOne({ messageBulkDeleteMode: !0 }), interaction.reply("\u2705 messageDeleteBulk has been enable.")),
			false === choices && (await guild_db.updateOne({ messageBulkDeleteMode: !1 }), interaction.reply("\u274C MessageDeleteBulk has been disable."));
		}
      ...
      : (await guild_db.updateOne({ welcomechannelID: e }), interaction.reply(`✅ Welcome Channel has been set to ${e}`));
		}
		...
		if (interaction.options.getSubcommand() === "modlog") {
			...
			isNaN(f) && "false" === i && (await guild_db.updateOne({ logchannelID: null })),
				isNaN(f) && "false" !== i
				? interaction.reply("\u274C You need to give me a channelID to set this setting.")
				: await guild_db.updateOne({ logchannelID: f }).then(() => interaction.reply(`✅ ModLog Channel has been set to ${f}`));
		}
		//ModRole ID Setup
        ...
        : (await guild_db.updateOne({ moderatorRoleID: h }), interaction.reply(`✅ ModRole has been set to ${h}`));
		}
    ...
    //**Need to be tested */
        ...
        : (await guild_db.updateOne({ PersonalizedWelcomeMessage: i }), interaction.reply(`✅ Welcome Message has been set to ${i}`));
		}
	}
};
