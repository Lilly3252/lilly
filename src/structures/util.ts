...
export default class Utils {
	...
	async loadCommands() {
		const slashyFiles = readdirSync(path.resolve(__dirnameForCommands, 'commands')).filter((file) =>
			file.toString().endsWith('.js'),
		);
		async () => {
			for (const commandFile of slashyFiles) {
				const slashy: SlashCommand = await import(
					pathToFileURL(path.resolve(__dirnameForCommands, 'commands', commandFile)).toString()
				);
	...
	async loadEvents() {
		const eventFiles = readdirSync(path.resolve(__dirnameForEvent, 'events')).filter((file) =>
			file.toString().endsWith('.js'),
		);
