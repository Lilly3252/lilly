...
export const run: SlashCommand['run'] = async (interaction: ChatInputCommandInteraction<'cached'>): Promise<any> => {
	switch (interaction.options.getSubcommand()) {
		case 'character': {
			...
			try {
				const resolvedCharacter = await fetch(`https://xivapi.com/character/search?name=${firstName}+${lastName}&server=${server}`).then((x) =>
					x.json(),
				);
				...
				if (jobs) {
					...
					const jobTransformed = jobAvailable.map((a) => jobClassData[a]);
