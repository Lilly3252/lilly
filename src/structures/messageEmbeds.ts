		...
		.addFields([
			...
			},
			{
				...
				value: [
					...
					`**❯ Roles [${role.length}]:** ${
						role.length < 10 ? role.join(', ') : role.length > 10 ? interaction.client.utils.trimArray(role) : 'None'
					}`,
					`\u200b`,
				].join('\n'),
			},
		...
		new discordJs.EmbedBuilder()
			...
			.setDescription(
				[
					...
					`**❯ ModRoleID:** ${guild_db.moderatorRole ? guild_db.moderatorRole : 'No information yet'} `,
					`**❯ WelcomeMessage:** ${
						guild_db.personalizedWelcomeMessage ? guild_db.personalizedWelcomeMessage : 'No information yet'
					}`,
			...
			.addFields([
				...
				},
				{
					...
					value: [
						...
						`**❯ Text Channels:** ${
							d.cache.filter((channel: discordJs.Channel) => channel.type === ChannelType.GuildText).size
						}`,
						`**❯ Voice Channels:** ${
							d.cache.filter((channel: discordJs.Channel) => channel.type === ChannelType.GuildVoice).size
						}`,
						`**❯ Stage Channels:** ${
							d.cache.filter((channel: discordJs.Channel) => channel.type === ChannelType.GuildStageVoice).size
						}`,
				...
				},
				{
					...
					value: [
						`**❯ Online:** ${
							member.cache.filter((guildmember: discordJs.GuildMember) => guildmember.presence?.status === 'online')
								.size
						}`,
						`**❯ Idle:** ${
							member.cache.filter((guildmember: discordJs.GuildMember) => guildmember.presence?.status === 'idle').size
						}`,
						`**❯ Do Not Disturb:** ${
							member.cache.filter((guildmember: discordJs.GuildMember) => guildmember.presence?.status === 'dnd').size
						}`,
						`**❯ Offline:** ${
							member.cache.filter((guildmember: discordJs.GuildMember) => guildmember.presence?.status === 'offline')
								.size
						}`,
						`**❯ No presence detected:** ${
							member.cache.filter((guildmember: discordJs.GuildMember) => guildmember.presence === null).size
						}`,
						'\u200B',
					].join('\n'),
				},
...
export function RoleEmbed(interaction: discordJs.ChatInputCommandInteraction<'cached'>, c: discordJs.Role) {
	return (
		new discordJs.EmbedBuilder()
			.setTimestamp()
			.setColor(c.color)
		...
		ChanEmbeds.addFields([
			{
				...
				value: [
					...
					`**❯ Private Channel:** ${
						channel.permissionsFor(interaction.guild.id)?.has(discordJs.PermissionsBitField.Flags.ViewChannel)
							? 'False'
							: 'True'
					}`,
				].join('\n'),
