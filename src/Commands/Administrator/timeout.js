...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("timeout").setDescription("timeout a member.").addUserOption((option) => option.setName("target").setDescription("Select a user").setRequired(true))
    .addStringOption((option) => option.setName("time").setDescription("time for timeout")
      .addChoices([
        ["5min", "300000"], ["10min", "600000"],
        ["15min", "900000"], ["30min", "1800000"],
        ["45min", "2700000"], ["1d", "86400000"],
        ["2d", "172800000"], ["3d", "259200000"],
        ["4d", "345600000"], ["5d", "432000000"],
        ["6d", "518400000"], ["1week", "604800000"],
        ["2weeks", "1209600000"], ["28days", "2419200000"]]).setRequired(true)
      .setRequired(true))
  ...
  async run(interaction) {
    ...
    member.timeout(+time, reason).then(() => {
      member.send(`Hello, you have been timeout in ${interaction.guild.name} for: ${reason}`)
        .catch((err) => console.log(err)),
        interaction.reply({ content: `${member.user.username} was successfully timeout.`, ephemeral: true });
    }).catch((err) => console.log(err));
